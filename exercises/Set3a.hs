...
firstHalf w = take ((length w + 1) `div` 2) w
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) . map (\d -> k ^ d) $ [0 ..]
...
joinToLength n xs | n <= 0 = []
joinToLength n xs = [res | x <- xs, y <- xs, let res = x ++ y, length res == n]
...
sumRights xs = sum $ fromRight [] $ sequence (filter isRight xs)
...
multiApp :: (Functor f) => (f b -> c) -> f (a -> b) -> a -> c
multiApp f fs v = f $ ($ v) <$> fs
...
interpreter = helper [0, 0]
 where
  helper res [] = []
  helper [x, y] ("up" : commands) = helper [x, y + 1] commands
  helper [x, y] ("down" : commands) = helper [x, y - 1] commands
  helper [x, y] ("right" : commands) = helper [x + 1, y] commands
  helper [x, y] ("left" : commands) = helper [x - 1, y] commands
  helper [x, y] ("printY" : commands) = show y : helper [x, y] commands
  helper [x, y] ("printX" : commands) = show x : helper [x, y] commands
  helper [x, y] (_ : commands) = error "not implemented"
