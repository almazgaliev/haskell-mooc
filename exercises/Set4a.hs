...
minmax :: (Ord a) => [a] -> (a, a)
minmax [] = undefined
minmax xs = helper (head xs) (head xs) xs
 where
  helper min' max' [] = (min', max')
  helper min' max' (x : xs)
    | x < min' = helper x max' xs
    | x > max' = helper min' x xs
    | otherwise = helper min' max' xs

rangeOf :: (Ord a, Num a) => [a] -> a
rangeOf xs = let (min', max') = minmax xs in max' - min'
...
-- on :: (a -> b) -> (b -> b -> c) -> c
f `on` g = \x y -> g y `f` g x

-- longest :: [[a]] -> [a]
-- longest = maximumBy (compare `on` length)
longest [] = []
longest (x : xs) =
  helper x (length x) xs
 where
  helper acc _ [] = acc
  helper acc len (x : xs)
    | xlen > len = helper x xlen xs
    | xlen < len = helper acc len xs
    | xlen == len && head x < head acc = helper x xlen xs
    | otherwise = helper acc len xs
   where
    xlen = length x
...
incrementKey k [] = []
incrementKey k (head'@(k', v') : kvs')
  | k == k' = (k, v' + 1) : incrementKey k kvs'
  | otherwise = head' : incrementKey k kvs'
...
freqs = helper (Map.fromList [])
 where
  helper map [] = map
  helper map (x : xs) = helper (Map.insert x (Map.findWithDefault 0 x map + 1) map) xs
