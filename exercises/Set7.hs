...
elem' x' = foldr (\x acc -> x' == x || acc) False
...
average (x :| xs) =
  let (sum, n) = foldl (\(sum, n) x -> (sum + x, n + 1)) (x, 1) xs
   in sum / n
...
reverseNonEmpty (x :| xs) = let (y : ys) = foldl (\acc x -> x : acc) [x] xs in y :| ys
...
passwordAllowed pwd (MinimumLength l) = length pwd >= l
passwordAllowed pwd (ContainsSome chars) = or [ch `elem` pwd | ch <- chars]
passwordAllowed pwd (DoesNotContain chars) = and [not (ch `elem` pwd) | ch <- chars]
...
data Arithmetic = Literal Integer | Add Arithmetic Arithmetic | Mult Arithmetic Arithmetic
  deriving (Show)
