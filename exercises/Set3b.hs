...
replicate' 0 elem = []
replicate' count elem = elem : replicate' (count - 1) elem

flip f a b = f b a

buildList :: Int -> Int -> Int -> [Int]
buildList = flip replicate''
 where
  replicate'' 0 elem last = [last]
  replicate'' count elem last = elem : replicate'' (count - 1) elem last
...
sums = sumsHelper 0 1
 where
  sumsHelper n inc i | inc == i = [n + inc]
  sumsHelper n inc i = (n + inc) : sumsHelper (n + inc) (inc + 1) i
...
-- (((ini + a) + b) + c)
myFoldl f ini [] = ini
myFoldl f ini (x : xs) = myFoldl f (ini `f` x) xs
...
sumsOf = helper 0
 where
  helper acc [] = []
  helper acc (x : xs) = let s = acc + x in s : helper s xs
...
mymaximum comparator initial = myFoldr (\x acc -> if x `comparator` acc then x else acc) initial
