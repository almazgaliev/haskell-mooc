...
oddFactorial n
 ...
 where
  oddFactorialHelper 1 = 1
  oddFactorialHelper n = n * oddFactorialHelper (n - 2)
...
countdown n = "Ready! " ++ concatMap ((++ "... ") . show) [n, (n - 1) .. 1] ++ "Liftoff!"
...
smallestDivisor x = findOr (\div -> x `mod` div == 0) x [2 .. (floor $ sqrt $ fromInteger x)]

findOr pred def cont = case find pred cont of
  Just val -> val
  Nothing -> def
